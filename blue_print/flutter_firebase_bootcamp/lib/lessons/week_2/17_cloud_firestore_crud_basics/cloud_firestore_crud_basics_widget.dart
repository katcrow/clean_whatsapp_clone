...
class _CloudFirestoreCrudBasicsWidgetState extends State<CloudFirestoreCrudBasicsWidget> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Cloud Firestore CRUD"),
      ),
      body: Column(
        children: [
          Center(
                  ...
                  children: users!.map((user) {
                    return ListTile(
                      ...
                      trailing: GestureDetector(
                          onTap: () {
                            _updateData(User(id: user.id, email: "aaa@gmail.com", username: "aaaa"));
                          },
  ...
  Stream<List<User>> _readData() {
    ...
    return userCollection.snapshots().map((querySnapshot) => querySnapshot.docs.map((e) => User.fromSnapshot(e)).toList());
  }

  _updateData(User user) {
    final userCollection = FirebaseFirestore.instance.collection("users");

    final newData = User(username: user.username, id: user.id, email: user.email).toJson();
  ...
  _createData(User user) {
    ...
    final newUser = User(username: user.username, id: id, email: user.email).toJson();
...
class User {
  ...
  static User fromSnapshot(DocumentSnapshot<Map<String, dynamic>> snapshot) {
    return User(username: snapshot['username'], email: snapshot['email'], id: snapshot['id']);
  }
