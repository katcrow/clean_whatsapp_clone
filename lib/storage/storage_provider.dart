...
class StorageProviderRemoteDataSource {
  ...
  static Future<String> uploadProfileImage({
    ...
    final ref = _storage.ref().child("profile/${DateTime.now().millisecondsSinceEpoch}");
  ...
  static Future<String> uploadStatus({
    ...
    final ref = _storage.ref().child("status/${DateTime.now().millisecondsSinceEpoch}");
  ...
  static Future<List<String>> uploadStatuses({
    ...
    for (var i = 0; i < files.length; i++) {
      final ref = _storage.ref().child("status/${DateTime.now().millisecondsSinceEpoch}${i + 1}");
  ...
  static Future<String> uploadMessageFile({
    ...
    final ref = _storage.ref().child("message/$type/$uid/$otherUid/${DateTime.now().millisecondsSinceEpoch}");
